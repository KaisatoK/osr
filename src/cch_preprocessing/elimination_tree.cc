...
namespace osr::cch_preprocessing {
  ...
  void elimination_tree::normal_contraction(graph& adj, std::vector<std::uint32_t>& res) {
    ...
    for (std::uint32_t i = 0; i < res.size(); ++i) {
      ...
      for (auto const& neighbor : adj[i]) {
        utl::verify(neighbor > i, "Neighbor {} is less than or equal to i {}", neighbor, i);
      }
  ...
  void elimination_tree::fast_contraction(graph const& adj, std::vector<std::uint32_t>& res) {
    // Initialize DSU
