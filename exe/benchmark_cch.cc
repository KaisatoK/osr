...
void set_start<car>(car::parameters const& params,
                    ...
                    ways const& w,
                    node_idx_t const start) {
  cch_q.add_start(car::node{start, 0, direction::kForward}, 0U);
  cch_q.add_start(car::node{start, 0, direction::kBackward}, 0U);
};
...
std::vector<typename P::label> set_end(typename P::parameters const& params,
  ...
  cch_q.add_dest(typename P::node{end});
  return {l};
}
...
std::vector<typename car::label> set_end<car>(car::parameters const& params,
  ...
  for (auto i = way_pos_t{0U}; i != ways.size(); ++i) {
    ...
    cch_q.add_dest(car::node{end, i, direction::kForward});
    cch_q.add_dest(car::node{end, i, direction::kBackward});
    ends.push_back(l1);
    ends.push_back(l2);
  }
  return ends;
