...
void valid_test(std::string_view data_dir, ways const& w) {
    ...
    auto const checkExists = [&](auto const from, auto const to, auto const cost) -> bool {
        bool found = false;
        if (from < to) {
            cc.for_each_edge<true>(cc.get_flatten_node_idx(from), [&](auto const&, auto const& e) {
                if (e.from_ == from && e.to_ == to && e.cost_ == cost) {
                    found = true;
                }
            });
        } else {
            cc.for_each_edge<false>(cc.get_flatten_node_idx(from), [&](auto const&, auto const& e) {
                if (e.from_ == from && e.to_ == to && e.cost_ == cost) {
                    found = true;
                }
            });
    ...
    for (std::uint32_t i = 0U; i < std::min(ordering.size(), size_t(20)); ++i) {
        fmt::print("{}'s parent is {}.\n", i, cc.specialized_elimination_tree_.at(i));
    }
    ...
    for (auto const& [idx, sub_idxes] : utl::enumerate(cc.virtual_nodes_)) {
        for (auto const& [sub_idx, node] : utl::enumerate(sub_idxes)) {
            ...
            if (sub_idx < sub_idxes.size() - 1U) {
            ...
            } else if (cc.upward_edges_.at(ext_node).first != cc.upward_edges_.at(ext_node).second) {
                ...
                utl::verify(par != node_idx_t::invalid(), "Parent node for extended node {} is missing", ext_node);
                utl::verify(min_node >= par.v_, "Parent node {} is less than minimal child node {} for extended node {}",
