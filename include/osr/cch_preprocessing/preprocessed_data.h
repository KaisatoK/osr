...
namespace osr::cch_preprocessing {
    ...
    std::variant<bool,
                 ...
                 cista::wrapped<customized_cost_stored<bike<bike_costing::kSafe, kElevationNoCost>>>,
                 cista::wrapped<customized_cost_stored<bike<bike_costing::kFast, kElevationNoCost>>>,
                 cista::wrapped<customized_cost_stored<bike<bike_costing::kSafe, kElevationLowCost>>>,
                 cista::wrapped<customized_cost_stored<bike<bike_costing::kSafe, kElevationHighCost>>>,
    ...
    struct preprocessed_data {
        ...
        static void load_customized_cost(std::filesystem::path const& dir) {
            utl::verify(std::filesystem::exists(dir), "preprocessed data directory does not exist");
            if (!forcedUpdate && std::holds_alternative<cista::wrapped<customized_cost_stored<P>>>(customized_cost_)) {
                return;
            }
        ...
        static customized_cost_stored<P> const& get_customized_cost() {
            utl::verify(std::holds_alternative<cista::wrapped<customized_cost_stored<P>>>(customized_cost_),
                        "customized cost for profile {} not loaded", profile_name<P>::value);
            return *std::get<cista::wrapped<customized_cost_stored<P>>>(customized_cost_);
        }
